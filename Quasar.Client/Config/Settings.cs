...
namespace Quasar.Client.Config
{
    ...
    public static class Settings
    {
        ...
        // Keylogger functionality has been completely removed
        ...
        {
            // If in portable mode, override paths to use local directories
            if (PORTABLE)
            {
                string appDir = Path.GetDirectoryName(Application.ExecutablePath) ?? Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData);
                string portableSettingsDir = Path.Combine(appDir, "portable_settings");
                
                DIRECTORY = appDir;
                LOGSPATH = Path.Combine(portableSettingsDir, "logs");
                INSTALLPATH = Path.Combine(appDir, INSTALLNAME);
            }
            else
            {
