...
namespace Quasar.Client.Setup
{
    public class ClientInstaller : ClientSetupBase
    {
        ...
        {
            // In portable mode, we don't do any of these operations
            if (PortableClientManager.IsPortableMode() || Settings.PORTABLE)
                return;
        ...
        public void Install()
        {
            // Skip installation if in portable mode
            if (PortableClientManager.IsPortableMode())
            {
                // No installation needed in portable mode
                return;
            }
